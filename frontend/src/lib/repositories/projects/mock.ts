...
export function createMockProjectsRepository(): ProjectsRepository {
  ...
  for (let i = 4; i <= 50; i++) {
    projects.push({
      id: i,
      name: `ダミープロジェクト ${String(i).padStart(2, "0")}`,
      description: i % 3 === 0 ? "検証用メモ" : null,
    });
  }
  ...
  chats[3] = [];
  // 各プロジェクトにダミーチャットを付与（0〜5件程度）
  for (let i = 4; i <= 50; i++) {
    const count = i % 6; // 0..5
    chats[i] = Array.from({ length: count }).map((_, idx) => ({
      id: crypto.randomUUID(),
      title: `検証チャット ${i}-${idx + 1}`,
    }));
  }
  return {
    ...
    // 補助: スレッドをこのプロジェクトのチャット一覧の先頭へ追加（モック用）
